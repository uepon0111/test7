...
function findBestMatchMusic(ocrText) {
  ...
  for (const music of dbMusics) {
    ...
    let score = dist / Math.max(target.length, dbTitleNorm.length);

    // 片方向の部分一致は、OCR の欠けや余計な文字に少し強くする
    if (target.includes(dbTitleNorm) || dbTitleNorm.includes(target)) {
      score *= 0.7;
    }

    // かな/全角半角の揺れなどで 1〜2 文字だけ崩れた場合の救済
    if (dist <= 2) {
      score *= 0.85;
    }
