...
function findBestProfileForImage(width, height, profiles) {
  ...
  if (!width || !height) return list[0] || null;
  ...
  for (const p of list) {
    ...
    const ratioDiff = Math.abs(pRatio - targetRatio) / targetRatio;

    const widthDiff = Math.abs(p.width - width) / Math.max(p.width, width);
    const heightDiff = Math.abs(p.height - height) / Math.max(p.height, height);
    const sizeDiff = (widthDiff + heightDiff) / 2;

    const score = ratioDiff * 0.65 + sizeDiff * 0.35;
