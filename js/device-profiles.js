 ...
 * 保存・読み込み・自動選択を担当します。index.html(メイン画面・読み取り設定モーダルの
 * 両方から)・ocr-analyzer.js から使用します。
 *
...
// プロファイルの regions から、欠けている項目をデフォルトで補完して返す。
// キー単位(difficulty/title/...)だけでなく、キーの中のフィールド単位でもマージする。
// これにより、
//   - 今回追加した 'level' region が無い古いプロファイル → デフォルトのlevel領域を補う
//   - 'binarize' フィールドを持たない古い region → デフォルトの 'auto' を補う
// といった後方互換を、将来項目が増えた場合も含めて自動的に扱える。
function getRegionsForProfile(profile) {
  if (!profile || !profile.regions) return cloneRegions(DEFAULT_REGIONS);
  const regions = {};
  for (const def of REGION_DEFS) {
    regions[def.key] = Object.assign({}, DEFAULT_REGIONS[def.key], profile.regions[def.key] || {});
  }
  return regions;
}
