...
function ensureProfile(deviceKey) {
  const key = normalizeDeviceKey(deviceKey);
  if (!state.settings.deviceProfiles[key]) {
    state.settings.deviceProfiles[key] = createDeviceProfile(key);
  }
  return state.settings.deviceProfiles[key];
}

function getCurrentDeviceKey() {
  return normalizeDeviceKey(state.settings?.currentDeviceKey || DEFAULT_DEVICE_KEY);
}

function getCurrentProfile() {
  return ensureProfile(getCurrentDeviceKey());
}

function getCropRegion(key, deviceKey = getCurrentDeviceKey()) {
  const profile = ensureProfile(deviceKey);
  return profile.cropRegions?.[key] || DEFAULT_CROP_SETTINGS[key];
}

function setCropRegion(key, nextRegion, deviceKey = getCurrentDeviceKey()) {
  const profile = ensureProfile(deviceKey);
  profile.cropRegions = profile.cropRegions || JSON.parse(JSON.stringify(DEFAULT_CROP_SETTINGS));
...
function getPreviewGeometry() {
  ...
  return { wrapRect, left, top, width, height };
}

function setCurrentProfileData(patch = {}) {
  const key = getCurrentDeviceKey();
  const profile = ensureProfile(key);
  state.settings.deviceProfiles[key] = {
    ...profile,
    ...patch,
    cropRegions: patch.cropRegions ? patch.cropRegions : profile.cropRegions,
  };
}
...
export function renderControlOptions() {
  ...
  if (sortSelect) {
    ...
    if (!sortSelect.value) sortSelect.value = "level_desc";
  }
  ...
  const deviceSelect = el("up-device");
  if (deviceSelect) {
    deviceSelect.innerHTML = Object.values(DEVICE_PRESETS).map((d) => `<option value="${d.key}">${escapeHtml(d.label)}</option>`).join("");
  }
...
function createCardElement(rec) {
  ...
  const thumb = rec.thumbnailSmall || rec.thumbnail || "";
  const large = rec.thumbnailFull || rec.thumbnail || "";
  ...
  card.innerHTML = `
    <div class="card-img-container">
      ${badge}
      ${pbBadge}
      <div class="card-overlay-actions ${state.isSelectMode ? "hidden" : ""}">
        <div class="btn-overlay" title="編集"><span class="material-symbols-outlined" data-action="edit">edit</span></div>
        <div class="btn-overlay del" title="削除"><span class="material-symbols-outlined" data-action="delete">delete</span></div>
      </div>
  ...
  const editBtn = card.querySelector('[data-action="edit"]')?.parentElement;
  const deleteBtn = card.querySelector('[data-action="delete"]')?.parentElement;
...
export async function renderGrid(records) {
  ...
  for (let i = 0; i < records.length; i += chunkSize) {
    ...
    for (const rec of slice) frag.appendChild(createCardElement(rec));
    grid.appendChild(frag);
    if (i + chunkSize < records.length) await nextFrame();
  }
}
...
function renderSettingsUI() {
  ...
  const deviceSelect = el("settings-device-select");
  if (deviceSelect) {
    deviceSelect.innerHTML = Object.values(DEVICE_PRESETS)
      .map((d) => `<option value="${d.key}">${escapeHtml(d.label)}</option>`)
      .join("");
    deviceSelect.value = getCurrentDeviceKey();
    deviceSelect.onchange = (ev) => {
      state.settings.currentDeviceKey = normalizeDeviceKey(ev.target.value);
      ensureProfile(state.settings.currentDeviceKey);
      persistSettings();
      renderSettingsUI();
    };
  }
  ...
  if (preview) {
    preview.src = getCurrentProfile().sampleImageDataUrl || "";
...
function updateSettingsPreview() {
  ...
  if (!layer || !img) return;
  layer.innerHTML = "";
  const geo = getPreviewGeometry();
  if (!geo) return;
  const deviceKey = getCurrentDeviceKey();
  regionKeys().forEach((key) => {
    const r = getCropRegion(key, deviceKey);
    const box = document.createElement("div");
    box.className = "crop-box";
    box.dataset.region = key;
    box.style.left = `${((geo.left - geo.wrapRect.left) + r.x * geo.width) / geo.wrapRect.width * 100}%`;
    box.style.top = `${((geo.top - geo.wrapRect.top) + r.y * geo.height) / geo.wrapRect.height * 100}%`;
    box.style.width = `${(r.w * geo.width) / geo.wrapRect.width * 100}%`;
    box.style.height = `${(r.h * geo.height) / geo.wrapRect.height * 100}%`;
    ...
    box.addEventListener("pointerdown", (ev) => beginCropDrag(ev, key));
    layer.appendChild(box);
  });
}

function beginCropDrag(ev, key) {
  ...
  const geo = getPreviewGeometry();
  if (!geo) return;
  const startRegion = { ...getCropRegion(key) };
  const startX = ev.clientX;
  const startY = ev.clientY;

  const onMove = (moveEv) => {
    const dx = (moveEv.clientX - startX) / geo.width;
    const dy = (moveEv.clientY - startY) / geo.height;
    const next = {
      ...startRegion,
      x: clamp(startRegion.x + dx, 0, 1 - startRegion.w),
      y: clamp(startRegion.y + dy, 0, 1 - startRegion.h),
    };
    setCropRegion(key, next);
    updateSettingsPreview();
    persistSettings();
  };
...
function resetSettings() {
  state.settings.deviceProfiles = {
    auto: createDeviceProfile("auto"),
  };
  state.settings.currentDeviceKey = DEFAULT_DEVICE_KEY;
  persistSettings();
  renderSettingsUI();
...
export function initializeUI() {
  ...
  window.updateView = applyFiltersAndSort;
}
...
export function selectItem(id) {
  ...
  el("up-device").value = normalizeDeviceKey(item.data.deviceKey || DEFAULT_DEVICE_KEY);
...
export function updateCurrentItem(field, value) {
  ...
  if (field === "deviceKey") {
    item.data.deviceKey = normalizeDeviceKey(value);
    item.data.manualDeviceKey = true;
    selectItem(item.id);
...
async function runBatchAnalysis(itemsToAnalyze) {
  ...
  for (const item of itemsToAnalyze) {
    ...
    try {
      ...
      const detectedKey = detectDeviceKeyFromRatio(img.naturalWidth, img.naturalHeight);
      if (!item.data.manualDeviceKey || !item.data.deviceKey || item.data.deviceKey === DEFAULT_DEVICE_KEY) {
        item.data.deviceKey = normalizeDeviceKey(detectedKey);
      }
      if (el("up-device") && state.activeItemId === item.id) el("up-device").value = item.data.deviceKey;

      const res = await analyzeLoadedImage(img, worker, item.data.deviceKey);
      if (res) {
        item.data = {
          title: res.title,
          level: res.level,
          diff: res.diff,
          deviceKey: item.data.deviceKey,
          perfect: res.perfect,
          great: res.great,
          good: res.good,
          bad: res.bad,
...
export async function batchEdit() {
  ...
  for (const rec of targets) {
    ...
    const highResUrl = rec.thumbnailFull || rec.thumbnail ? (rec.thumbnailFull || rec.thumbnail).replace("=s220", "=w1600") : "";
    ...
    state.editorQueue.push({
      ...
      data: {
        ...
        deviceKey: rec.deviceKey || DEFAULT_DEVICE_KEY,
        manualDeviceKey: !!rec.deviceKey,
