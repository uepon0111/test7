...
function parseAppProperties(props = {}) {
  ...
  return {
    ...
    imageWidth: num(props.imageWidth),
    imageHeight: num(props.imageHeight),
  };
}
...
function buildRecordFromDriveFile(file, folderMap) {
  ...
  const record = {
    ...
    imageWidth: app.imageWidth || 0,
    imageHeight: app.imageHeight || 0,
  };
...
function buildFileMetadata(item, parentId) {
  const appProperties = {
    ...
    modelLabel: item.data.modelLabel || "",
    imageWidth: String(item.data.imageWidth || 0),
    imageHeight: String(item.data.imageHeight || 0),
  };
