import { ROOT_FOLDER_NAME, RESULT_SUBFOLDER_NAME, buildSongFolderName, normalizeDifficultyToken, getDifficultyOrderKey, normalizeDeviceKey } from "./config.js";
...
function transformThumbnailUrl(url, size) {
  if (!url) return null;
  const u = String(url);
  if (u.includes("=s")) return u.replace(/=s\d+/, `=w${size}`);
  if (u.includes("=w")) return u.replace(/=w\d+/, `=w${size}`);
  return `${u}=w${size}`;
}
...
function parseAppProperties(props = {}) {
  ...
  return {
    ...
    deviceKey: props.deviceKey || null,
  };
}
...
function buildRecordFromDriveFile(file, folderMap) {
  ...
  const thumbnailSmall = transformThumbnailUrl(file.thumbnailLink, 420);
  const thumbnailFull = transformThumbnailUrl(file.thumbnailLink, 1600);
  ...
  const record = {
    ...
    totalMiss: missCount,
    isFC: missCount === 0,
    thumbnail: thumbnailSmall || file.thumbnailLink || null,
    thumbnailSmall,
    thumbnailFull,
    ...
    deviceKey: normalizeDeviceKey(app.deviceKey) || null,
  };
...
function buildFileMetadata(item, parentId) {
  const appProperties = {
    ...
    deviceKey: item.data.deviceKey || "auto",
  };
