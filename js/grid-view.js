...
// 既にフィルタ・ソート済みのリストを、フィルタ処理をやり直さずに再描画するための軽量ラッパー。
// 選択モードの切り替え・選択解除など、絞り込み結果は変えずに見た目(選択枠など)だけ
// 更新したい場面(selection-mode.js)で使う。renderToken は据え置きなので、他の
// updateView() 実行中の描画を誤ってキャンセルすることはない。
// (以前は selection-mode.js からこの名前の関数が呼ばれているにもかかわらず未定義で、
//  選択モードの切り替え時にエラーになっていたための修正)
...
// 楽曲マスターDB(musics.json / musicDifficulties.json)の取得に失敗した場合の警告バナー。
// DBが無いと曲名の自動判定・総合判断(result-resolver.js)が機能しないため、
// 気づかないまま使い続けることがないよう明示的に知らせる。
function showDbLoadWarning() {
  const area = document.getElementById('notification-area');
  area.style.display = 'flex';
  const el = document.createElement('div');
  el.className = 'best-notification is-error';
  el.innerHTML = `
    ...
    <div class="best-notification-body">
      ...
      <div class="best-notification-detail">曲名・レベルの自動判定や整合性チェックが正しく機能しない可能性があります。時間をおいてページを再読み込みしてください。</div>
    </div>
    <button type="button" class="best-notification-close" aria-label="閉じる" onclick="this.closest('.best-notification').remove(); hideNotificationAreaIfEmpty();">&times;</button>
  `;
  area.appendChild(el);
