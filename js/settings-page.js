 ...
 * settings-page.js
 * -----------------------------------------------------------------------
 * 「読み取り設定」モーダルのロジック。以前は settings.html という別ページでしたが、
 * ページ遷移すると Google ログインの状態(アクセストークン)がメモリ上にしか保持されて
 * おらず失われてしまう問題があったため、index.html 内のモーダルに統合しました。
 * これにより設定を開いても一切ページ遷移が発生せず、ログインが切れることはありません。
 *
 ...
 *   - 二値化の極性(自動/文字が濃い色/文字が薄い色)の項目ごとの指定
...
// モーダルを開く。既にDOM上に組み立て済みの内容を表示するだけなので、非表示中は
// クライアント幅が0になり範囲ボックスの位置計算がずれている可能性があるため、
// 表示した直後に必ず repositionAllRegionBoxes() で位置を再計算する。
...
function initSettingsPanel() {
  ...
  document.getElementById('coord-binarize').addEventListener('change', onBinarizeChanged);
...
function populateBinarizeSelect() {
  const sel = document.getElementById('coord-binarize');
  sel.innerHTML = BINARIZE_MODES.map(m => `<option value="${m.key}">${escapeHtml(m.label)}</option>`).join('');
}
...
function loadProfileIntoEditor(profile) {
  ...
  // 旧バージョンで保存されたプロファイルには今回追加した 'level' 領域や 'binarize' フィールドが
  // 存在しないため、デフォルト値で補完してから編集対象にする(補完しないと、後段の
  // renderRegionBoxPosition('level') などが region=undefined を参照してエラーになる)。
  currentProfile.regions = getRegionsForProfile(currentProfile);
...
function onRegionPointerMove(ev) {
  ...
  currentProfile.regions[dragState.key] = Object.assign({}, currentProfile.regions[dragState.key], { x, y, w, h });
...
function updateCoordInputsFromRegion(key) {
  ...
  document.getElementById('coord-binarize').value = region.binarize || 'auto';
...
function onCoordInputChanged() {
  ...
  currentProfile.regions[activeRegionKey] = Object.assign({}, currentProfile.regions[activeRegionKey], { x, y, w, h });
  renderRegionBoxPosition(activeRegionKey);
}

// 二値化の極性(自動/文字が濃い色/文字が薄い色)を切り替える。
function onBinarizeChanged() {
  if (!currentProfile) return;
  const value = document.getElementById('coord-binarize').value;
  currentProfile.regions[activeRegionKey] = Object.assign({}, currentProfile.regions[activeRegionKey], { binarize: value });
}
