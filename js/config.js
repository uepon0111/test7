...
export const LEGACY_STORAGE_KEY_SETTINGS = "prsk-result-viewer-settings-v2";
...
export function createProfileId(width = 0, height = 0) {
  const w = Number(width) || 0;
  const h = Number(height) || 0;
  return `profile_${w || "x"}x${h || "x"}_${Date.now().toString(36)}`;
}
