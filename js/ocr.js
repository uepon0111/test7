...
export function detectDifficultyFromText(text) {
  ...
  if (/APPEND|AP+END/.test(t)) return "append";
...
export function getCropSettingsForModel(modelId) {
  const profile = getProfile(state.settings, modelId);
  return profile?.cropRegions || state.settings?.cropRegions || DEFAULT_CROP_SETTINGS;
}

export async function analyzeLoadedImage(imgElement, worker, modelId = null) {
  try {
    const settings = getCropSettingsForModel(modelId || state.settings?.activeModelId);
    ...
    const title = matchedMusic ? matchedMusic.title : String(titleRet.data.text || "").replace(/?
/g, " ").trim();
