import { DEFAULT_CROP_SETTINGS, getDifficultyLabel } from "./config.js";
...
export function getCropSettingsForDevice(deviceKey) {
  const profile = state.settings?.deviceProfiles?.[deviceKey] || state.settings?.deviceProfiles?.auto;
  return profile?.cropRegions || DEFAULT_CROP_SETTINGS;
}

export async function analyzeLoadedImage(imgElement, worker, deviceKey = "auto") {
  try {
    const settings = getCropSettingsForDevice(deviceKey);
