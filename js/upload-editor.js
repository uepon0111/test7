  ...
  div.innerHTML = `
      ...
      <div class="sidebar-info">
          ...
          <div class="sidebar-status">
              ...
              <span id="sb-warn-${id}" class="sidebar-warn-badge" style="display:none;" title="確認が必要な警告があります">
...
function updateCurrentItem(field, value) {
  ...
  if (['good', 'bad', 'missDetail', 'perfect', 'great', 'combo', 'diff'].includes(field)) {
...
function updateSidebarStatus(id) {
  ...
  const warnEl = document.getElementById(`sb-warn-${id}`);
  if (warnEl) {
    const hasWarnings = !!(item.analysis && item.analysis.warnings && item.analysis.warnings.length > 0);
    warnEl.style.display = hasWarnings ? 'inline-flex' : 'none';
  }
}
  ...
  for (const item of itemsToAnalyze) {
    ...
    try {
      ...
      if (res) {
        ...
        item.analysis = res; // 実測ログ(debugLog)・警告・候補一覧などを保持しておく
        item.status = 'done';
      } else {
        item.analysis = null;
        item.status = 'error';
